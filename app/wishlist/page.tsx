        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-black text-white sm:text-4xl flex items-center gap-3">
              <Heart className="h-7 w-7 text-[#ff003c] fill-current" />
              SAVED <span className="text-[#ff003c]">WISHLIST</span> ({wishlist.length})
            </h1>
          ...
          {wishlist.length > 0 ? (
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {wishlist.map((product) => (
                <div
                  key={product.id}
                  className="group relative flex flex-col justify-between rounded-2xl border border-[#22222e] bg-[#0e0e12] p-4 transition hover:border-[#ff003c]/60 glass-panel"
                >
                  <div className="space-y-3">
                    ...
                    <div className="relative h-48 w-full overflow-hidden rounded-xl bg-[#050505] p-2">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 rounded-full bg-[#050505]/80 p-2 text-[#ff003c] hover:bg-[#ff003c] hover:text-white transition"
                    ...
                    <div>
                      <span className="text-[10px] font-extrabold text-[#ff003c] uppercase">{product.brand}</span>
                      <h3 className="text-sm font-bold text-white line-clamp-1">{product.name}</h3>
                      <div className="flex items-center gap-1 text-xs text-[#ffb800] mt-1">
                  ...
                  <div className="mt-4 pt-3 border-t border-[#1f1f2b] flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="red-gradient-btn flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-xs font-bold text-white shadow-lg shadow-[#ff003c]/20"
                    >
