        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-black text-white sm:text-4xl flex items-center gap-3">
              <Scale className="h-7 w-7 text-[#3B82F6]" />
              PRODUCT <span className="text-[#3B82F6]">COMPARISON MATRIX</span> ({compareList.length}/4)
            </h1>
                    ...
                    {compareList.map((item) => (
                      <th key={item.id} className="p-4 text-left font-extrabold text-white min-w-64">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-[#3B82F6] uppercase">{item.brand}</span>
                          <button
                            onClick={() => removeFromCompare(item.id)}
                            className="text-[#71717a] hover:text-[#3B82F6]"
                          >
                    ...
                    {compareList.map((item) => (
                      <td key={item.id} className="p-4">
                        <button
                          ...
                          className="red-gradient-btn flex items-center justify-center gap-1.5 w-full rounded-xl py-2.5 text-xs font-bold text-white shadow-lg shadow-[#3B82F6]/20"
                        >
