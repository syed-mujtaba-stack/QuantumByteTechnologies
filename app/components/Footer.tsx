...
const socialLinks = [
  { Icon: SocialIcons.WhatsApp,  href: 'https://wa.me/923254803957', label: 'WhatsApp',  color: 'hover:text-[#25D366] hover:border-[#25D366]/25' },
  { Icon: SocialIcons.Twitter,   href: '#',                          label: 'X/Twitter', color: 'hover:text-[#60A5FA] hover:border-[#60A5FA]/25' },
  { Icon: SocialIcons.Instagram, href: '#',                          label: 'Instagram', color: 'hover:text-[#F472B6] hover:border-[#F472B6]/25' },
  { Icon: SocialIcons.Youtube,   href: '#',                          label: 'YouTube',   color: 'hover:text-[#F87171] hover:border-[#F87171]/25' },
  { Icon: SocialIcons.Linkedin,  href: '#',                          label: 'LinkedIn',  color: 'hover:text-[#60A5FA] hover:border-[#60A5FA]/25' },
  { Icon: SocialIcons.Github,    href: '#',                          label: 'GitHub',    color: 'hover:text-white hover:border-white/20' },
];
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="flex w-full items-center justify-between py-3 lg:py-0 lg:mb-4 text-left lg:cursor-default"
      >
        <h4 className="text-[11px] font-bold text-white uppercase tracking-[0.12em]">{title}</h4>
        <ChevronDown
          className={`h-3.5 w-3.5 text-[#475569] transition-transform duration-300 lg:hidden ${open ? 'rotate-180' : ''}`}
        />
      </button>

      <ul
        role="list"
        className={`space-y-2 overflow-hidden transition-all duration-300 lg:!max-h-none lg:!opacity-100 ${
          open ? 'max-h-96 opacity-100 pb-3' : 'max-h-0 opacity-0 lg:pb-0'
        }`}
      >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        ...
        <div className="grid gap-8 lg:gap-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-12">
          ...
          <div className="sm:col-span-2 lg:col-span-4 space-y-5">
            ...
            <p className="text-[12.5px] leading-relaxed text-[#64748B] max-w-[280px]">
              Your high-performance hardware retailer and certified enterprise IT solution provider.
              Custom gaming rigs, laptops, mobiles, chargers, genuine parts, and micro-electronics repair.
            </p>

            <div className="inline-flex items-center gap-1.5 rounded-lg border border-white/[0.05] bg-white/[0.02] px-2.5 py-1.5">
              <ShieldCheck className="h-3 w-3 text-[#3B82F6] shrink-0" />
              <span className="text-[11px] font-medium text-[#64748B]">Official Registered Tech Brand</span>
            </div>

            {/* Contact */}
            <div className="space-y-2">
              <a href="tel:+923254803957" className="flex items-center gap-2.5 text-[12.5px] text-[#64748B] hover:text-white transition-colors duration-200 group">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/[0.03] border border-white/[0.05] group-hover:border-[#3B82F6]/20 transition-colors shrink-0">
                  <Phone className="h-3 w-3 text-[#475569]" />
                </div>
                +92 325 4803957
              </a>
            ...
            <div className="flex items-center gap-1.5">
              {socialLinks.map(({ Icon, href, label, color }) => (
                <a
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.02] text-[#475569] transition-all duration-200 hover:bg-white/[0.04] ${color}`}
                >
                  <Icon />
                </a>
              ))}
          ...
          <div className="sm:col-span-2 lg:col-span-2">
            <h4 className="text-[11px] font-bold text-white uppercase tracking-[0.12em] mb-3">Stay Updated</h4>
            <p className="text-[12.5px] text-[#64748B] mb-4 leading-relaxed">
              Hardware drops, price alerts &amp; tech news.
            </p>
            <form onSubmit={handleNewsletterSubmit} className="space-y-2">
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#475569]" aria-hidden="true" />
                <input
                  type="email"
                  required
                  ...
                  className="w-full h-9 rounded-xl border border-white/[0.06] bg-white/[0.025] pl-9 pr-3 text-[12.5px] text-white placeholder-[#475569] outline-none transition-all duration-200 focus:border-[#3B82F6]/40 focus:bg-white/[0.04] focus:shadow-[0_0_0_3px_rgba(59,130,246,0.08)]"
                />
              </div>
              <button
                type="submit"
                disabled={subscribed}
                className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-[#3B82F6] to-[#2563EB] h-9 text-[12.5px] font-bold text-white transition-all duration-200 hover:shadow-[0_0_16px_rgba(59,130,246,0.3)] active:scale-[0.98] disabled:opacity-50"
              >
                <Send className="h-3 w-3" />
                {subscribed ? 'Subscribed!' : 'Subscribe'}
              </button>
            </form>
            <p className="text-[10px] text-[#475569] mt-2.5">Mon–Sat, 10AM–8PM PKT</p>
          </div>
        </div>
            ...
            <nav className="flex flex-wrap items-center justify-center gap-3 sm:gap-4" aria-label="Legal links">
              {footerLinks.legal.map(({ label, href }) => (
                <Link
                  key={href}
                  href={href}
                  className="text-[11.5px] text-[#475569] hover:text-white/80 transition-colors duration-200"
                >
                  {label}
                </Link>
              ))}
