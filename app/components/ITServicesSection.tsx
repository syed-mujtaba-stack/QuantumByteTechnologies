...
export function ITServicesSection({ itServices }: ITServicesSectionProps) {
  ...
  return (
    <section id="services-section" className="relative border-b border-[#22222e] bg-[#050505] py-20 lg:py-28 cyber-grid-bg">
      <div className="pointer-events-none absolute left-1/2 top-1/2 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#ff003c]/10 blur-[160px]" />

      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          <GSAPFadeIn direction="down" delay={0.1}>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#ff003c]/40 bg-[#ff003c]/10 px-4 py-1.5 text-xs font-extrabold text-white">
              <Wrench className="h-3.5 w-3.5 text-[#ff003c]" />
          ...
          <GSAPFadeIn direction="up" delay={0.2}>
            <h2 className="text-3xl font-black text-white sm:text-4xl lg:text-5xl">
              ENTERPRISE-GRADE <span className="text-[#ff003c] glow-red-text">IT SOLUTIONS</span>
            </h2>
          </GSAPFadeIn>
        ...
        <div className="mt-14 grid gap-8 md:grid-cols-2">
          {itServices.map((service, index) => (
            <GSAPFadeIn key={service.id} direction="up" delay={0.2 + index * 0.1}>
              <GSAPHoverTilt className="h-full">
                <div className="group relative flex h-full flex-col justify-between rounded-2xl border border-[#22222e] bg-[#0e0e12] p-6 transition-all duration-300 hover:border-[#ff003c]/60 hover:bg-[#12121c] hover:shadow-2xl hover:shadow-[#ff003c]/15 glass-panel">
                    ...
                    <div className="space-y-2 pt-2 border-t border-[#1f1f2b]">
                      {service.features.map((feat, idx) => (
                        <div key={idx} className="flex items-center gap-2.5 text-xs text-white">
                          <CheckCircle2 className="h-4 w-4 flex-shrink-0 text-[#ff003c]" />
                          <span>{feat}</span>
                        </div>
                      ))}
                    </div>
